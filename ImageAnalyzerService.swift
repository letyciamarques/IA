...
public class ImageAnalyzer: ImageAnalyzerService {
    ...
    public func analyzeImage(image: UIImage, navigationController: UINavigationController, failure: @escaping (_ error: String) -> Void) {
        ...
        guard let model = try? VNCoreMLModel(for: analiseHamsterGato(configuration: MLModelConfiguration()).model) else {
            failure("can't load analiseHamsterGato ML model")
            return
        }
        ...
        // Run the Core ML analiseHamsterGato classifier on global dispatch queue
